...
class UNetBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, activation=nn.ReLU(), batchnorm=True):
        super(UNetBlock, self).__init__()
        layers = [nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=not batchnorm)]
        if batchnorm:
            layers.append(nn.BatchNorm2d(out_channels, 0.5))
        layers.append(activation)
        layers.append(nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, bias=not batchnorm))
        if batchnorm:
            layers.append(nn.BatchNorm2d(out_channels, 0.5))
        layers.append(activation)
...
class UpBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, activation=nn.ReLU(inplace=True)):
...
class UNetAuto(nn.Module):
    def __init__(self, num_channels: int, activation=nn.ReLU) -> None:
        ...
        self.core_input = UNetBlock(num_channels, f_thickness, self.activation, batchnorm=False)
        ...
        self.bottom_layer = UNetBlock(f_thickness*8, f_thickness*16, self.activation, batchnorm=False)
...
class UNetAutoSmall(nn.Module):
    def __init__(self, num_channels: int, activation=nn.ReLU()) -> None:
        ...
        self.bottom_layer = UNetBlock(f_thickness*4, f_thickness*8, self.activation, batchnorm=False)
        self.up_depth_2 = UpBlock(f_thickness*8, f_thickness*4, self.activation)
        self.up_depth_1 = UpBlock(f_thickness*4, f_thickness*2, self.activation)
        self.out_depth = UpBlock(f_thickness*2, f_thickness, self.activation)
